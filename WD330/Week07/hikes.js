...
class Comments {
  ...
  showCommentsList() {
    ...
    renderCommentList(this.parentElement, this.getAllComments(), "hikes");
  }
...
function renderCommentList(parent, comments, comType) {
  comments[comType].forEach(comment => {
    parent.appendChild(renderOneComment(comment));
  });
}
function renderOneComment(comment) {
  const item = document.createElement('li');
  item.innerHTML = `<div>${comment.name}</div>
